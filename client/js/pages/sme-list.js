...
export default async function renderSmeList(container) {
  ...
  function render(smes) {
    ...
    document.getElementById('sme-table').innerHTML = `<div class="card"><div class="table-wrap"><table><thead><tr><th>ID</th><th>Name</th><th>Role</th><th>Email</th><th>Department</th><th>Status</th></tr></thead><tbody>
      ${smes.length === 0 ? `<tr><td colspan="6" style="text-align:center;padding:32px;color:var(--text-secondary)">No SMEs found</td></tr>` :
      smes.map(s => `<tr style="cursor:pointer" onclick="window.location.hash='#/sme/${s.sme_id}'">
        <td><code>${s.sme_id}</code></td><td><strong>${s.full_name}</strong></td><td>${s.role || '-'}</td><td>${s.contact_json?.email || '-'}</td><td>${s.department || '-'}</td>
