...
const conflictCreateSchema = Joi.object({
  type: Joi.string().valid('process_discrepancy','data_inconsistency','technology_mismatch','ownership_dispute','other').required(),
  ...
  related_process_ids_json: Joi.alternatives().try(Joi.array(), Joi.string()).optional(),
  related_gap_ids_json: Joi.alternatives().try(Joi.array(), Joi.string()).optional()
});
